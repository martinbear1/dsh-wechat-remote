...
正式三端参照基线固定为：DSH 插件 `1.5.5`、微信小程序 `1.6.4`、云端中继 `1.2.2`。候选插件必须保留该基线已经验证的公网批传输、冻结小程序协议和云端透明转发边界；DSH 新旧版本差异只能在电脑插件适配层内收敛。

## 当前验证矩阵

| 操作系统 | DSH 0.1.1-rc.2 | DSH 0.1.2-alpha.1 | DSH 0.1.2-alpha.2 | DSH 0.1.2-alpha.3 |
| --- | --- | --- | --- | --- |
| Windows | 通过 | 通过 | 通过 | 通过 |
| macOS | 通过 | 通过 | 待回归 | 待回归 |
...
- 在独立临时会话发送最小提示，并在 `events.mux` 收到首个 `assistant/chunk`

- 连续正文必须产生归并后的实时 delta，并最终收到 `turn/end`。

2026-09-01 的 Windows 实机门禁中，DSH `0.1.2-alpha.2` 与 `0.1.2-alpha.3` 均使用同一候选插件完成了上述真实模型流；归并事件分别覆盖 5 与 7 个原始 DSH 序号。
...
- `npm run smoke:installed-gate:deep`：在指定测试目录创建临时工作区和会话，验证读写及实时事件后清理；增加 `--prompt-check true` 时必须收到首个实时 `assistant/chunk`。
