...
当前源码为 `1.7.7-rc.2` 预览版，配套安装器同号。以下 `@latest` 命令安装已发布的正式版；体验 RC2 请使用配套预览安装包。
...
- 此前版本的安装与自动重启已验证：DSH `0.1.5-rc.1`，Windows x64、macOS Intel x64、Linux x64 和 Linux ARM64。RC2 的具体验证范围随预览版说明提供，不以历史记录代替。未登记的 CPU 或 DSH 版本不会仅因缺少测试记录而被禁止安装或更新。
...
插件与 npm 安装工具本版统一为 `1.7.7-rc.2`。需要绕过安装器时，可参考 [DSH 官方原生安装方式](docs/NATIVE-INSTALL.md)。开发与验收记录见 [1.7.7 工作记录](docs/INSTALLER-1.7.7-WORKLOG.md)。
