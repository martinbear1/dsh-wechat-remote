...
当前源码为 `1.7.7-rc.3` 预览检查点，配套安装器同号，目前仅保存本地制品。以下 `@latest` 命令安装已发布的正式版。
...
- 此前版本的安装与自动重启已验证：DSH `0.1.5-rc.1`，Windows x64、macOS Intel x64、Linux x64 和 Linux ARM64。RC3 本轮仅完成本地构建与回归，未进行真机安装验收，不以历史记录代替。未登记的 CPU 或 DSH 版本不会仅因缺少测试记录而被禁止安装或更新。
...
插件与 npm 安装工具本版统一为 `1.7.7-rc.3`。需要绕过安装器时，可参考 [DSH 官方原生安装方式](docs/NATIVE-INSTALL.md)。开发与验收记录见 [RC3 检查点记录](docs/RESEARCH-RC3-CHECKPOINT-20260917.md)。
