...
本工作树是 `1.6.0-dev.20260905.1` 开发候选，基于正式版 `1.5.5`，尚未发布。下方原生安装命令仍指向已发布代码，不会安装本地开发包。
...
测试的 DSH 为 `0.1.1-rc.1`、`0.1.1-rc.2`、`0.1.2-rc.1`。它们均为 RC；不代表整个区间、其他安装方式或 Apple Silicon 已获验证。自动事务使用隔离合成节点；公网正式清单和 GitHub 下载到按钮更新的发布级联调另行验收。
