...
- 基于 RC 的正式候选保留全部通知源码；通知 12 项、全量原有插件回归、更新策略 24 项、安全 17 项、选择 12 项及发现 17 项（Windows 跳过 POSIX 链接专属项）通过。
...
- 最终报告目录分别为 `npx-native-proof-pJRPNf`、`npx-native-proof-1aWMcm`、`npx-native-proof-gpBvct`。旧运行节点、配对和全局 DSH 安装未改动。
- 初次 ARM 故障注入暴露：候选插件加载失败时，DSH 的退出可能挂起，SIGTERM 后未退出导致备份无法复位。现在只对本次事务持有句柄的新候选进程，在等待正常退出超时后发送 SIGKILL，再验证退出后恢复；绝不强杀发现到的 PID 或用户原宿主。实机回退复测结果完成后记录。
- 云端 115 项与小程序 12 项关联测试通过；小程序零改动。云端程序改动仅共享更新策略，通知/配对/支付/客服/中继其他源码保持不变。
- 候选插件 9548268 字节，SHA256 `e4c9d393aef7b304f26efc89f87f83c1ebfa1a775d27cad3a3fc8d7fd8d312ad`；npm 包 18508331 字节，SHA256 `82b38d0d41bb36992b021c05b49a4abdf45ac33985df91e1aae7992646f19120`。内嵌插件与独立归档逐字节一致。发布后必须再次下载核对。
