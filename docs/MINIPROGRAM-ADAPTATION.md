...
## 3. 旧配置（:3090）迁移：**最终拍板 = 强硬方案**

v1 的两个方案（自动替换 / 强制跳初始化页）与小程序侧 f6c86bc 的软迁移（黄字提示）
均已被最终决策取代（2026-08-18，用户拍板）：

- **冷启动检测到旧配置（baseUrl 含 `:3090`）→ 直接进初始化页，视为新用户重新
  扫码配对**；不自动替换端口（旧 token 对新网关无效，替换只会全线 401），
  不保留"继续用旧通道"路径 —— 借此还原「新用户第一次使用」的完整流程。
- 判定触发点：`store.loadConfig().baseUrl` 含 `:3090` 时，bootstrap 返回
  初始化页路由（与"未配置"同路径，仅提示文案不同：检测到旧版配置）。
