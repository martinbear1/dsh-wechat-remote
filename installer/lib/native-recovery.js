import { createRequire as recoveryRequire } from 'node:module'; const require = recoveryRequire(import.meta.url);
...
// installer/bin/native-control.mjs
...
function writePatch(filename, text) {
  ...
  const mode = fs.existsSync(filename) ? fs.statSync(filename).mode : 384;
  try {
...
function document(text) {
  const doc = (0, import_yaml.parseDocument)(text, { customTags: [{ tag: "tag:yaml.org,2002:js", resolve: (value) => value }] });
  if (doc.errors.length || !(0, import_yaml.isSeq)(doc.contents)) throw new Error("DSH \u914D\u7F6E\u4E0D\u662F\u6709\u6548\u7684\u539F\u751F\u63D2\u4EF6\u5217\u8868\uFF0C\u672A\u4FEE\u6539\u914D\u7F6E\u3002");
  return doc;
}
function attachControl(profile, helper, config) {
  ...
  fs.writeFileSync(path.join(config.directory, "user-patch-before.yml"), original, { mode: 384, flag: "wx" });
  writePatch(filename, patched);
  return () => {
    const current = fs.readFileSync(filename, "utf8");
    if (digest(current) === digest(patched)) {
      writePatch(filename, original);
      return;
    }
    const latest = document(current);
    latest.contents.items = latest.contents.items.filter((item) => {
      if (!(0, import_yaml.isMap)(item)) return true;
      const entries = item.get("insert", true);
      if (!(0, import_yaml.isSeq)(entries)) return true;
      entries.items = entries.items.filter((entry) => !(0, import_yaml.isMap)(entry) || entry.get("id") !== id);
      return entries.items.length > 0 || item.items.length !== 1;
    });
    writePatch(filename, String(latest));
  };
}
async function waitForJson(filename, accept, timeout = 2e4) {
...
async function verifyNativeRestore(job, start, sessionIds, readableIds) {
  ...
  const helper = path2.join(directory, "install-control.js"), token = randomBytes2(24).toString("hex");
  fs2.copyFileSync(path2.join(job.directory, "install-control.js"), helper);
  ...
  const request = async (method, payload = {}) => {
    ...
    }, body: JSON.stringify(payload), redirect: "error", signal: AbortSignal.timeout(12e3) });
