...
const ROOT = path.join(dshDataHome(), 'harness-remote');
...
export function agentProfileScope() {
    ...
    const match = /[\\/]profiles[\\/]([^\\/]+)[\\/]node_modules[\\/]/i.exec(modulePath);
...
export function defaultAgentIdentityPath() {
    ...
    if (scope === 'web' || scope === 'default') {
        return path.join(dshDataHome(), 'harness-remote-public-identity.json');
    }
    // Keep the pre-DSH_HOME per-profile key so existing cloud ownership and
    // pairings survive the storage-root correction without re-enrollment.
