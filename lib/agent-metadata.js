...
export function resolveAgentProfileScope(modulePath, argv, dshHome) {
    ...
    if (valid(parts[0]) && parts[1] === 'node_modules')
        return parts[0];
    if (argv[2] === 'web')
        return 'web';
    return 'default';
 ...
 */
export function gateStatePathForProfile(profileScope, homeDirectory = homedir(), dshHome = path.join(homeDirectory, '.dsh')) {
    ...
    return path.join(dshHome, 'harness-remote', 'instances', instanceStorageKey(profileScope), 'gate-wechat-state.json');
}
export function defaultGateStatePath() {
