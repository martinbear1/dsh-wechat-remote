...
export function assistantAttemptPresentation(event) {
    if (event.type !== 'assistant/attempt' || !Array.isArray(event.data?.stream))
        return undefined;
    const blocks = new Map();
    const add = (index, type, text, replace = false) => {
        if (!Number.isSafeInteger(index) || index < 0 || index > 4096 || typeof text !== 'string')
            return;
        const old = blocks.get(index);
        if (replace || !old || old.type !== type)
            blocks.set(index, { type, parts: [text] });
        else
            old.parts.push(text);
    };
    for (const record of event.data.stream) {
        if (record.type === 'text-chunks' || record.type === 'reasoning-chunks') {
            for (const text of record.texts || [])
                add(record.index, record.type === 'text-chunks' ? 'text' : 'reasoning', text);
        }
        else if (record.type === 'chunk') {
            const c = record.chunk || {};
            if (c.type === 'text-delta' || c.type === 'reasoning-delta')
                add(c.index, c.type === 'text-delta' ? 'text' : 'reasoning', c.text);
            else if (c.type === 'block-start' && (c.blockType === 'text' || c.blockType === 'reasoning'))
                add(c.index, c.blockType, '', true);
            else if (c.type === 'block-end' && (c.block?.type === 'text' || c.block?.type === 'reasoning'))
                add(c.index, c.block.type, c.block.text, true);
        }
    }
...
export function assistantRecordPresentation(entry) {
...
export function firstCompactTokenTime(stream) {
    if (!Array.isArray(stream))
        return undefined;
    for (const record of stream) {
        if (record.type === 'chunk') {
            const c = record.chunk || {};
            if ((c.type === 'text-delta' || c.type === 'reasoning-delta') && c.text || c.type === 'tool-call-delta' && (c.argumentsDelta || c.name !== undefined))
                return record.time;
        }
            ...
            for (let i = 0; i < texts.length; i++) {
                if (i)
                    time += record.dt?.[i - 1] || 0;
                if (texts[i] || record.type === 'tool-call-chunks' && record.name !== undefined)
                    return time;
            }
        }
    }
...
export class AssistantStreamCompatibility {
    ...
    reset() {
        ...
        for (const index of this.blocks)
            this.emit({ type: 'assistant/chunk', data: {
    ...
    chunk(chunk, time) {
        if (!chunk || typeof chunk.type !== 'string')
            throw new Error('Invalid assistant stream chunk');
        const index = Number.isSafeInteger(chunk.index) ? chunk.index : 0;
        if (index < 0 || index > 4096)
            throw new Error('Invalid assistant block index');
        this.blocks.add(index);
        this.emit({ type: 'assistant/chunk', ...(time === undefined ? {} : { time }), data: { chunk } });
    }
    baseline(value) {
        if (!value)
            return; // Older RCs retain durable assistant/chunk events.
        this.reset();
        ...
        for (const entry of active?.stream || []) {
            if (entry.type === 'chunk')
                this.chunk(entry.chunk, entry.time);
            else if (entry.type === 'text-chunks' || entry.type === 'reasoning-chunks') {
                if (!Array.isArray(entry.texts) || !entry.texts.every((text) => typeof text === 'string'))
    ...
    follow(frame) {
        if (!frame || !Number.isSafeInteger(frame.revision)
            || frame.revision !== (this.revision ?? 0) + 1)
            throw new Error('Assistant stream revision gap');
        ...
        if (frame.type === 'start') {
            ...
            if (frame.attemptId !== this.attempt || frame.index !== this.index)
                throw new Error('Assistant stream attempt/index gap');
            if (frame.type === 'chunk') {
                this.chunk(frame.chunk, frame.time);
                this.index++;
            }
            else {
