...
export class DshCompatibilityApi {
    ...
    dshPort;
    maintaining;
    /** Absent unless the isolated research feature was explicitly enabled. */
    ...
    handlesPath(path) {
        if (this.taskNotificationRequest && path === '/api/wechatTaskNotifications')
            return true;
        if (this.maintaining())
            return true;
    ...
    async dispatch(request) {
        ...
        try {
            ...
            if (legacy.method === 'session.prompt' && Array.isArray(legacy.payload.content)) {
                if (legacy.payload.content.filter(part => part?.type === 'file').length > 6)
                    throw Error('每条消息最多 6 个文件');
                const inputs = this.ctx.get('agentInputs');
                legacy.payload.content = legacy.payload.content.map(part => {
                    if (part?.type !== 'file' || !part.inputToken)
                        return part;
                    if (!inputs)
