    ...
    async consumeSessions(generation, signal) {
        ...
        for (const item of result.value.items) {
            if (typeof item?.sessionId === 'string')
                this.followSession(item.sessionId, generation, signal);
        }
    }
    followSession(sessionId, generation = this.generation, signal) {
        if (!sessionId || this.followedSessions.get(sessionId) === generation)
            return;
        ...
        void (async () => {
            ...
            try {
                ...
                if (this.followedSessions.get(sessionId) === generation)
                    this.followedSessions.delete(sessionId);
            }
        })();
    }
