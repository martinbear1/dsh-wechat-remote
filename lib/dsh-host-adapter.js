        ...
        try {
            ...
            if (method === 'session.history') {
                ...
                if (result.ok && typeof payload.sessionId === 'string') {
                    ...
                    await waitWithSignal(this.eventsHub.ensureSessionFollow(payload.sessionId, legacyHistoryTail(result.value?.events)), signal);
                }
                return result;
            }
...
class ModernLegacyEventHub {
    ...
    pending = new Map();
    queueBaselines = new Map();
    projectionBaselines = new Map();
    sessionHeads = new Map();
    sessionSummaries = new Map();
    desiredSessions = new Set();
    liveDeltas = new Map();
    clientId = '';
    started = false;
    generation = 0;
    ...
    async respond(rpcId, result, signal) {
        ...
        if (pending.responding)
            return { accepted: false, reason: 'response-in-flight' };
        ...
        try {
            ...
            if (this.pending.get(rpcId) === pending)
                pending.responding = false;
        }
    }
    observeSessionCreated(value) {
        if (!isRecord(value))
            return;
    ...
    ensureSessionFollow(sessionId, throughSeq) {
        if (!sessionId)
            return Promise.reject(new Error('Session follow requires sessionId'));
    ...
    refreshSessionFollow(sessionId) {
        if (!sessionId)
            return Promise.reject(new Error('Session follow requires sessionId'));
        this.desiredSessions.add(sessionId);
        this.ensureGenerationActive();
    ...
    reconcileSessionCatalog(items) {
        ...
        for (const item of items) {
            ...
            if (!previous)
                this.host({ type: 'host/session-added', ...item });
    ...
    start() {
        if (this.started || this.restartTimer || this.lifetime.signal.aborted)
            return;
        this.started = true;
        this.generation += 1;
    ...
    async runGeneration(generation, signal) {
        try {
            ...
            if (!signal.aborted)
                throw new Error('DSH modern compatibility streams ended');
        }
        catch (error) {
    ...
    failGeneration(generation, error) {
        if (generation !== this.generation || !this.started || this.lifetime.signal.aborted)
            return;
        ...
        const delay = Math.min(STREAM_RESTART_BASE_MS * (2 ** Math.min(this.restartAttempt, 8)), STREAM_RESTART_MAX_MS);
    ...
    async consumeRemoteEvents(signal) {
        ...
        if (!signal.aborted)
            throw new Error('DSH $events stream ended');
    }
    consumeEmit(event, args) {
        if (event === 'api-session/added' && isRecord(args[0])) {
            ...
            const previous = this.sessionSummaries.get(sessionId);
            if (previous)
                this.sessionSummaries.set(sessionId, { ...previous, running: Boolean(args[1]) });
    ...
    async consumeWorkspace(signal) {
        ...
        if (!signal.aborted)
            throw new Error('DSH workspace stream ended');
    }
    async consumeControl(signal) {
        ...
        for await (const value of stream) {
            ...
            if (value.type === 'baseline' && isRecord(value.value)) {
                ...
                for (const sessionId of this.queueBaselines.keys()) {
                    if (!Object.hasOwn(queues, sessionId))
                        this.cacheQueue(sessionId, []);
                }
        ...
        if (!signal.aborted)
            throw new Error('DSH session control stream ended');
    }
    async consumeSessions(generation, signal) {
        ...
        for (const sessionId of this.desiredSessions) {
            if (available.has(sessionId))
                void this.followSession(sessionId, generation, signal);
        }
    }
    followSession(sessionId, generation = this.generation, signal, replace = false) {
        if (!sessionId)
            return Promise.reject(new Error('Session follow requires sessionId'));
        const previous = this.followedSessions.get(sessionId);
        if (previous && previous.generation === generation && !replace)
            return previous.ready;
        if (previous)
            this.stopSessionFollow(sessionId, 'DSH Session follow refreshed');
        const controller = new AbortController();
        ...
        void (async () => {
            ...
            try {
                ...
                for await (const value of stream) {
                    ...
                    if (!opened && value.type === 'snapshot') {
                        ...
                        if (Number.isSafeInteger(value.cursor)) {
                            ...
                            if (!readySettled)
                                resolveReady(Number(value.cursor));
                        }
                        else
                            throw new Error(`DSH Session snapshot cursor is invalid for ${sessionId}`);
                ...
                if (!followSignal.aborted)
                    throw new Error(`DSH Session stream ended for ${sessionId}`);
            }
            catch (error) {
                if (!readySettled)
                    rejectReady(error);
                ...
                if (this.followedSessions.get(sessionId) === lease)
                    this.followedSessions.delete(sessionId);
            }
        })();
        return ready;
    }
    stopSessionFollow(sessionId, reason) {
    ...
    cacheProjection(sessionId, key, value, seq) {
        if (!sessionId || !key || !Number.isSafeInteger(seq))
            return;
        ...
        if (previous && Number(seq) < previous.seq)
            return;
    ...
    replayMux(queue) {
        ...
        for (const pending of this.pending.values()) {
            if (!pending.responding)
                this.push(queue, pending.payload, pending.rpcId);
        }
    }
    broadcastResolved(pending, result) {
     ...
     */
    publishSessionEvent(sessionId, event, view) {
    ...
    flushAllSessionDeltas() {
        for (const sessionId of Array.from(this.liveDeltas.keys()))
            this.flushSessionDelta(sessionId);
    }
    commitSessionHead(sessionId, event) {
    ...
    push(queue, payload, rpcId) {
        const bytes = this.encode(payload, rpcId);
        if (bytes)
            queue.push(bytes);
    }
    encode(payload, rpcId) {
...
function legacyHistoryEntries(records, viewState = createLegacyViewState()) {
 ...
 */
function legacyMutationView(event, state) {
    ...
    if (event.type === 'tool/call') {
        ...
        if (callId)
            state.mutationPaths.set(callId, mutationPath(data.name, data.arguments));
        return undefined;
    }
    ...
    if (!path || firstContent?.isError === true)
        return undefined;
    return { view: { card: 'diff', diffs: [{ path }] } };
}
function mutationPath(name, argumentsRaw) {
    if (typeof name !== 'string' || typeof argumentsRaw !== 'string')
        return null;
    let args;
    try {
 ...
 */
function coalesceChunkRowEvent(event) {
    ...
    if (!Array.isArray(members) || members.length === 0 || !Array.isArray(data.dt))
        return null;
...
function eventRangeEnd(event) {
    return Number.isSafeInteger(event.seqEnd) && Number(event.seqEnd) >= Number(event.seq)
        ? Number(event.seqEnd)
        : Number(event.seq);
}
function legacyHistoryTail(entries) {
    if (!Array.isArray(entries))
        return undefined;
    let tail = -1;
    for (const entry of entries) {
        if (!isRecord(entry) || !isRecord(entry.event) || !Number.isSafeInteger(entry.event.seq))
            continue;
...
function liveDeltaDescriptor(event) {
    if (event.type !== 'assistant/chunk' || !Number.isSafeInteger(event.seq)
        || !isRecord(event.data) || !isRecord(event.data.chunk))
        return null;
    ...
    if (!field || typeof chunk[field] !== 'string')
        return null;
    const key = [
