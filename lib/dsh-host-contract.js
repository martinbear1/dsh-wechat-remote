...
export function runningDshVersion(entry = process.argv[1]) {
    if (!entry)
        return;
    let directory;
    try {
...
export function openHostEvents(gateway, endpoint, payload, signal) {
...
export async function invokeHostRemote(ctx, gateway, request) {
    ...
    if (registry?.get('subagents/list') || registry?.hasSeen?.('subagents/list') || !usesDuplexEvents(runningDshVersion()))
        return gateway.invoke(request);
