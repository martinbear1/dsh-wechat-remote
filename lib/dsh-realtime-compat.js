...
export class DshRealtimeCompatibility {
    ...
    async respond(value) {
        ...
        if (this.responding.has(target))
            return { accepted: false, reason: 'response is already being delivered' };
        ...
        try {
            ...
            if (this.pending.get(rpcId)?.includes(target))
                this.settlePending(rpcId);
    ...
    run(state, task, signal = state.lifetime.signal, allowCompletion = false) {
        void task().then(() => {
            if (!allowCompletion && !signal.aborted && !this.disposed)
    ...
    startSession(state, sessionId) {
        ...
        const follow = async () => {
            try {
                ...
                const assistant = new AssistantStreamCompatibility(event => this.send(state, { type: 'session/event', sessionId, event }));
                ...
                for await (const raw of iterable) {
                    ...
                    if (frame?.type === 'snapshot') {
                        ...
                        const resources = resourcePresentation(frame.event);
                        const turnActivity = activity.accept(frame.event);
                        const event = frame.event;
                        rememberCall(event);
                        let entry = assistantRecordPresentation({ event,
                        ...
                        const history = this.ctx.get('wechatHistory');
                ...
                if (!combined.aborted)
                    throw new Error('DSH Session stream ended unexpectedly');
            }
            catch (error) {
                if (combined.aborted)
                    return;
                if (!isSessionReadError(error) && error?.code !== 'history-detail-unavailable')
                    throw error;
    ...
    async followRemoteEvents(state) {
        ...
        for await (const raw of iterable) {
            ...
            if (frame.type === 'waterfall')
                ...
                this.settlePending(stringOf(frame.eventId));
        }
    }
    pendingWaterfall(state, frame) {
        ...
        if (values.some(value => value.clientId === clientId))
            return;
        values.push(pending);
