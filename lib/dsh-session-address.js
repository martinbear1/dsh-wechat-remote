 ...
 */
export async function resolveDshSessionAddress(gateway, sessionId, signal) {
    signal.throwIfAborted();
    if (!sessionId || sessionId.length > 256)
        unavailable('会话标识无效', sessionId, 'invalid-id');
    ...
    if (!Array.isArray(listing?.items))
        unavailable('DSH 会话目录暂不可用', sessionId, 'invalid-directory');
    const rows = listing.items.map(record).filter(row => row?.sessionId === sessionId);
    if (rows.length !== 1)
        unavailable('会话不存在或暂不可读取，请刷新会话列表', sessionId, 'missing-session');
    const row = rows[0];
    ...
    if (!Array.isArray(catalog?.entries))
        unavailable('DSH 子代理目录暂不可用', sessionId, 'invalid-catalog');
    const children = catalog.entries.map(record).filter(entry => entry?.id === sessionId);
    if (children.length !== 1)
        unavailable('子代理会话暂不可读取，请刷新后重试', sessionId, 'missing-child');
    const child = children[0];
    if (child.kind !== 'child' || (child.mode !== 'one-shot' && child.mode !== 'continuable')) {
        unavailable('DSH 无法识别此子代理会话，请在电脑端检查该会话', sessionId, typeof child.reason === 'string' ? child.reason : 'unsupported-child');
    }
...
export function isSessionReadError(error) {
