...
export class DshTunnelAgent {
    ...
    streams = new Map();
    sendQueue;
    get pendingSendBytes() { return this.sendQueue.bytes; }
    closed = false;
    constructor(options) {
        ...
        this.sendQueue = new TunnelSendQueue({
            ...
            onDrain: () => { if (!this.closed && this.pendingSendBytes <= SEND_QUEUE_RESUME_BYTES)
                this.resumeSources(); },
            onOverflow: id => {
    ...
    openHttp(streamId, path, value) {
        ...
        const request = http.request({
            ...
            if (active?.kind !== KIND_HTTP || this.closed) {
                response.destroy();
                return;
            }
            ...
            response.on('data', chunk => {
                for (const part of pieces(Buffer.from(chunk))) {
                    if (this.streams.get(streamId) !== active || this.closed)
                        return;
                    this.queue(encode(DATA, streamId, 0, part));
                }
        ...
        const stream = { kind: KIND_HTTP, path, request, paused: false, bytes: 0 };
    ...
    openCompatibilityEvents(streamId, path, value) {
        const stream = {
        ...
        const detach = this.compatibilityApi.connectEvents(path, {
            ...
            send(message) {
                if (tunnel.streams.get(streamId) === stream && !tunnel.closed) {
                    if (!accepted)
                        openingMessages.push(message);
                    else
                        tunnel.sendWebSocketMessage(streamId, stream, Buffer.from(message), false);
                }
            },
            close(code, reason) {
                if (tunnel.streams.get(streamId) !== stream)
                    return;
        ...
        if (this.streams.get(streamId) !== stream)
            ...
            for (const message of openingMessages)
                this.sendWebSocketMessage(streamId, stream, Buffer.from(message), false);
        }
    }
    ...
    data(stream, streamId, flags, payload) {
        if (stream.kind === 'compat-events')
            throw new Error('DSH compatibility events are downlinks only');
        if (stream.kind === 'compat-http') {
            if (stream.ended)
                throw new Error('DSH request has already ended');
    ...
    async forwardRemotePrompt(streamId, stream) {
        ...
        try {
            ...
            if (active)
                owner = active;
            if (!active || (active.kind !== KIND_HTTP && active.kind !== 'compat-http'))
    ...
    resumeSources() {
        for (const stream of this.streams.values()) {
            if ((stream.kind !== KIND_HTTP && stream.kind !== KIND_WEBSOCKET) || !stream.paused)
                continue;
            stream.paused = false;
            if (stream.kind === KIND_HTTP)
    ...
    async forwardCompatibilityHttp(streamId, stream) {
        ...
        try {
            ...
            if (signal.aborted || this.closed || this.streams.get(streamId) !== stream)
                return;
