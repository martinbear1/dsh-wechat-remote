...
function uint64(counter) {
    if (!Number.isSafeInteger(counter) || counter < 0)
        throw new Error('E2EE counter exhausted');
...
function readUint64(data) {
    if (data.length !== 8)
        throw new Error('Invalid E2EE counter');
...
function transcriptHash(nodeId, clientHello, agentUnsigned) {
    return nacl.hash(concat(utf8('HarnessRemote-E2EE-v1\0'), utf8(nodeId), new Uint8Array([0]), clientHello, agentUnsigned));
}
function derive(shared, transcript) {
...
export class AgentE2EESession {
    ...
    rxCounter = 0;
    constructor(options) {
        if (!/^[A-Za-z0-9_-]{20,64}$/.test(options.nodeId))
            throw new Error('Invalid Agent nodeId');
        const secret = options.randomSecret || randomBytes(32);
        const prefix = options.randomPrefix || randomBytes(16);
        if (secret.length !== 32 || prefix.length !== 16)
            throw new Error('Invalid E2EE random material');
    ...
    receive(rawPacket) {
        ...
        if (this.state === 'waiting-client')
            return this.receiveClientHello(packet);
        ...
        const clear = this.openPacket(packet);
        if (clear[0] !== CLEAR_DATA)
            throw new Error('Invalid E2EE clear-frame type');
    ...
    seal(data) {
        ...
        if (clear.length > MAX_CLEAR_BYTES)
            throw new Error('E2EE clear frame exceeds 1 MiB');
        return this.sealClear(concat(new Uint8Array([CLEAR_DATA]), clear));
    }
    receiveClientHello(packet) {
        ...
        const agentUnsigned = concat(new Uint8Array([PACKET_AGENT_HELLO, VERSION]), this.keyPair.publicKey, this.sendPrefix);
        ...
        if (signature.length !== nacl.sign.signatureLength)
            throw new Error('Invalid Agent identity signature');
    ...
    sealClear(clear) {
        if (!this.txKey)
            throw new Error('E2EE send key is unavailable');
        const counterBytes = uint64(this.txCounter);
        this.txCounter += 1;
        const nonce = concat(this.sendPrefix, counterBytes);
        return concat(new Uint8Array([PACKET_SEALED, VERSION]), counterBytes, nacl.secretbox(clear, nonce, this.txKey));
    }
    openPacket(packet) {
        if (!this.rxKey || !this.receivePrefix)
            throw new Error('E2EE receive key is unavailable');
        if (packet.length < 27 || packet[0] !== PACKET_SEALED || packet[1] !== VERSION)
            throw new Error('Invalid E2EE sealed packet');
        const counterBytes = packet.subarray(2, 10);
        if (readUint64(counterBytes) !== this.rxCounter)
            throw new Error('E2EE replay or out-of-order packet');
