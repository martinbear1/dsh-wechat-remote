...
export function mountWechatGate(ctx) {
    ...
    if (loadedState.warning)
        console.warn(`[wechat-gate] ${loadedState.warning}`);
    ...
    function synchronizeLanIdentity(nodeId) {
        if (!lanStatePersistent)
            return;
        if (state.publicIdentityNodeId === nodeId)
            return;
     ...
     */
    async function serveUpdateVerificationProbe(req, res) {
    ...
    async function makePairEntry() {
        ...
        const locatorPayload = () => JSON.stringify({ v: 1, mode: 'secure-lan-route',
        ...
        try {
            ...
            console.warn('[wechat-gate] public pairing ticket unavailable; serving identity-pinned route locator:', messageOf(error));
        }
...
<button type="button" onclick="location.reload()">重新生成二维码</button>
    ...
    disableLanDoor = () => {
        ...
        doorRuntime.publicDoor.errorCode = 'GATE_STATE_UNAVAILABLE';
        doorRuntime.publicDoor.message = '局域网凭据文件无法安全读写，已暂停局域网入口';
        for (const client of secureLan.sockets.clients)
            client.terminate();
        try {
            publicServer.close();
        }
        catch { /* already closed or not listening */ }
    };
    publicServer.on('upgrade', (req, socket, head) => {
        ...
        if (req.url === '/wechat-remote/secure-lan') {
            secureLan.sockets.handleUpgrade(req, socket, head, ws => secureLan.attach(ws, req.socket.remoteAddress));
            return;
        }
    ...
    const historyConfig = {
        dshPort: UPSTREAM_PORT,
        storeSnapshot: async (payloadJson, archive, signal) => {
            const gateway = publicRelayGateway;
            if (!gateway)
    ...
    mountChild('agent-resources', AgentResourcesService, {
        store: async (data, signal) => {
            if (!publicRelayGateway)
                throw new Error('公网文件服务暂不可用，请连接局域网后重试');
    ...
    mountChild('agent-inputs', AgentInputsService, {
        load: async (descriptor, signal) => {
            if (!publicRelayGateway)
                throw Error('公网附件服务暂不可用，请连接局域网后重试');
        ...
        if (relayConfig) {
            publicRelayGateway = new PublicRelayGateway(relayConfig, {
                ...
                issueLanCredential: (rotate = false) => {
                    ...
                    if (!lanStatePersistent)
                        throw new Error('局域网凭据状态不可用，请先修复凭据文件');
            ...
            if (process.env.HR_TASK_NOTIFICATIONS_ENABLED !== '0') {
                try {
                    taskNotifications = new TaskNotifications(ctx, new NotificationRelayClient(relayConfig.relayOrigin, () => { if (!publicRelayGateway)
                        throw new Error('Node unavailable'); return publicRelayGateway.agent.identity; }));
    ...
    if (!lanStatePersistent) {
        ...
        try {
            publicServer.listen(PUBLIC_PORT, '0.0.0.0', () => {
                ...
                console.log(`[wechat-gate] encrypted LAN door: ws://0.0.0.0:${PUBLIC_PORT}/wechat-remote/secure-lan`);
            });
        }
        catch (error) {
