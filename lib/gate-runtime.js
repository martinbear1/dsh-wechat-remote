...
export function mountWechatGate(ctx) {
    ...
    const selectedGatePorts = deriveGatePorts(PROFILE_SCOPE, agentDescriptor.agentInstanceId, process.env);
    ...
    function accessLog(req, note) {
        try {
            ...
            fs.appendFileSync(path.join(PROFILE_ROOT, 'wechat-gate-access.log'), line);
        }
        catch (e) {
...
<p>${entry.publicMode ? '自动选择更快连接；远程内容端到端加密' : '当前仅支持同一网络连接'}</p>
    ...
    async function serveThroughHostAdapter(req, res) {
        const chunks = [];
        let bytes = 0;
        for await (const chunk of req) {
            ...
            if (bytes > 64 * 1024 * 1024)
                throw new Error('DSH request exceeds 64 MiB');
            chunks.push(part);
        }
        const controller = new AbortController();
        const close = () => controller.abort(new Error('LAN client disconnected'));
        ...
        try {
            ...
            response.headers.forEach((value, key) => {
                if (!['connection', 'transfer-encoding', 'set-cookie'].includes(key))
                    headers[key] = value;
            });
    ...
    const publicServer = http.createServer((req, res) => {
        ...
        if (hostAdapter.usesModernTransport) {
            void serveThroughHostAdapter(req, res).catch((error) => {
                if (!res.headersSent)
                    res.writeHead(502, { 'Content-Type': 'text/plain' });
                if (!res.destroyed)
                    res.end(`Bad Gateway: ${messageOf(error)}`);
            });
            return;
        }
                ...
                void (async () => {
                    try {
                        ...
                        if (!controller.signal.aborted)
                            client.close(1011, messageOf(error).slice(0, 100));
                    }
                })();
            });
            return;
    ...
    try {
        ...
        if (relayConfig) {
            publicRelayGateway = new PublicRelayGateway(relayConfig, {
                ...
                historyCachePath: path.join(PROFILE_ROOT, `wechat-history-snapshots-${agentDescriptor.agentInstanceId}.json`),
