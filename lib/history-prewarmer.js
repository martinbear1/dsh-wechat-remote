 ...
 */
export function bindHistorySnapshotPrewarmer(ctx, options) {
...
export class HistorySnapshotPrewarmer {
    ...
    connect() {
        if (this.stopped || this.socket || this.releaseObserver)
            return;
    ...
    observe(raw) {
        ...
        if (frame.type !== 'host/session-status' || typeof frame.running !== 'boolean')
            return;
        const wasRunning = this.running.get(sessionId);
        this.running.set(sessionId, frame.running);
        if (wasRunning === true && frame.running === false)
    ...
    enqueue(entry) {
        if (this.stopped || this.queued.has(entry.sessionId))
            return;
        ...
        if (this.queue.length >= this.maxQueue) {
            ...
            if (discarded)
                this.queued.delete(discarded.sessionId);
        }
    ...
    forget(sessionId) {
        ...
        if (this.active?.sessionId === sessionId)
            this.active.controller.abort(new Error('Session removed'));
    }
...
function parseHostFrame(raw) {
    try {
        ...
        if (data.byteLength === 0 || data.byteLength > 1024 * 1024)
            return null;
        const root = JSON.parse(data.toString('utf8'));
