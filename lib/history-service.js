...
export class LatestHistoryWindowCache {
    ...
    capture(request) {
        if (!this.tracking || request.beforeSeq !== undefined)
            return null;
        return {
            epoch: this.epoch,
    ...
    write(request, payloadJson, token) {
        if (!token || !this.tracking || request.beforeSeq !== undefined
            || token.epoch !== this.epoch
            || token.revision !== (this.revisions.get(request.sessionId) || 0))
            return false;
...
let WechatHistoryService = (() => {
    ...
    return class WechatHistoryService extends _classSuper {
        ...
        prepareSnapshot;
        latestCache = new LatestHistoryWindowCache();
        ...
        async window(request, signal) {
            ...
            try {
                ...
                if (cached !== null)
                    return this.deliver(cached, request, true);
        ...
        async deliver(payloadJson, request, cacheHit) {
 ...
 */
export async function prewarmLatestHistory(service, sessionId, signal) {
    ...
    if (!result.value.snapshotJson)
        return 'inline';
    const descriptor = JSON.parse(result.value.snapshotJson);
...
function latestCacheKey(request) {
