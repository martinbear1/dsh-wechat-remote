...
let WechatHistoryService = (() => {
    ...
    return class WechatHistoryService extends _classSuper {
        ...
        async window(request, signal) {
            ...
            try {
                const built = await buildHistoryWindow(request, (payload, pageSignal) => (this.fetchNativePage(payload, pageSignal)), signal, await nativeTurnUsage());
                if (!built.ok)
                    return built;
        ...
        async deliver(payloadJson, request) {
            ...
            return {
                ...
                value: {
                    ...
                    events: compactEntries(pages.flat(), completedTurns, durableMessageTurns).map(entry => {
                        ...
                        const attempt = assistantAttemptPresentation((entry.event || {}));
                        ...
                        return resources || attempt || turnActivity ? { ...projected, view: { ...(entry.view || {}),
