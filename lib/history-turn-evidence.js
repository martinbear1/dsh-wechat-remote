...
const pick = (value, names) => Object.fromEntries(names.filter(key => value?.[key] !== undefined).map(key => [key, value[key]]));
...
function factOf(event) {
    const d = event.data || {}, data = pick(d, ['turn', 'step', 'callId', 'name']);
    const message = (m = {}) => ({ ...pick(m, ['id', 'role', 'isError', 'toolCallId']),
        source: pick(m.source, ['kind', 'callId', 'provider', 'model']),
        content: [...new Map((Array.isArray(m.content) ? m.content : []).filter(Boolean).map((b) => {
    ...
    const usage = (v) => pick(v, ['inputTokens', 'outputTokens', 'cacheReadTokens', 'cacheWriteTokens', 'reasoningTokens', 'totalTokens']);
    if (event.type === 'user/message')
        Object.assign(data, message(d));
    if (d.message)
        data.message = message(d.message);
    if (d.reason)
        data.reason = pick(d.reason, ['kind']);
    if (d.error)
        data.error = true;
    if (d.usage)
        data.usage = usage(d.usage);
    ...
    if (Array.isArray(d.stream)) {
        const sample = d.stream.findLast((r) => r.type === 'chunk' && r.chunk?.type === 'usage');
        if (sample)
            data.stream = [{ ...pick(sample, ['type', 'time']), chunk: { type: 'usage', usage: usage(sample.chunk.usage) } }];
    }
...
export class HistoryTurnEvidence {
    ...
    accept(sessionId, entries, usageFold) {
        ...
        if (window && (through + 1 < window.from || from > window.through + 1
            || additions.some(item => { const old = window.facts.get(item.fact.event.seq); return old && old.json !== item.json; })))
            window = undefined;
        window ??= { from, through, facts: new Map(), bytes: 0 };
        for (const item of additions)
            if (!window.facts.has(item.fact.event.seq)) {
                window.facts.set(item.fact.event.seq, item);
                window.bytes += Buffer.byteLength(item.json);
            }
        this.sessions.delete(sessionId);
        if (window.facts.size > MAX_EVENTS || window.bytes > MAX_BYTES)
            return none;
        window.from = Math.min(window.from, from);
        window.through = Math.max(window.through, through);
        ...
        const activity = new TurnActivityCompatibility(), facets = [], firstTokens = new Map();
        for (const fact of facts) {
            const view = activity.accept(fact.event, { mutationPath: fact.mutation });
            if (view)
                facets.push(view);
            if (fact.firstToken !== undefined)
                firstTokens.set(fact.event.seq, fact.firstToken);
        }
