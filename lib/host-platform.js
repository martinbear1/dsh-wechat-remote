...
function scoreAddress(candidate) {
    ...
    if (/^(en0|en1|wi-?fi|wlan\d*|ethernet|以太网)/i.test(name))
        score += 30;
    if (/(docker|veth|vmnet|virtualbox|hyper-v|wsl|loopback|utun|bridge|tunnel|vpn)/i.test(name))
        score -= 80;
    if (address.startsWith('169.254.') || isBenchmarkAddress(address) || isCarrierGradeNat(address))
        score -= 200;
    return score;
}
