    ...
    try {
        ...
        if (relayConfig) {
            publicRelayGateway = new PublicRelayGateway(relayConfig, {
                ...
                historyCachePath: path.join(os.homedir(), '.dsh', `wechat-history-snapshots-${agentDescriptor.agentInstanceId}.json`),
            ...
            historySnapshotPrewarmer = new HistorySnapshotPrewarmer({
                ...
                warm: async (sessionId, signal) => {
                    if (!ctx.wechatHistory)
                        throw new Error('Wechat history service is unavailable');
