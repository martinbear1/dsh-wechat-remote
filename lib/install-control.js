...
export async function nativeInstallSessions(ctx, signal) {
    const controller = ctx.get('sessionController');
    if (typeof controller?.list !== 'function')
        throw new Error('DSH 原生会话检查不可用，未停止节点。');
    signal.throwIfAborted();
    const value = await controller.list({}, signal);
    signal.throwIfAborted();
    if (!Array.isArray(value?.items) || value.items.some((s) => typeof s.sessionId !== 'string'
        || !s.sessionId || typeof s.running !== 'boolean')
        || new Set(value.items.map((s) => s.sessionId)).size !== value.items.length) {
        throw new Error('DSH 原生会话状态不完整，未停止节点。');
    }
    return value;
}
export async function quiesceNativeHost(ctx, read, disposing) {
    const items = (await read('session.list')).items;
    if (!Array.isArray(items) || items.some((s) => s.running !== false))
        throw new Error('请等待运行中的会话结束后再更新。');
    const sessions = ctx.get('sessions');
    if (!sessions?.list || !sessions.flush)
        throw new Error('当前 DSH 不支持保存检查。');
    for (const session of sessions.list())
        if (!await sessions.flush(session))
            throw new Error('会话保存未完成。');
    if ((await read('session.list')).items.some((s) => s.running !== false))
        throw new Error('有新会话开始运行，请稍后重试。');
    disposing();
    await ctx.fiber.dispose();
}
export async function createInstallControl(context, config) {
    ...
    if (!/^[a-f0-9]{32}$/.test(id) || !/^[a-f0-9]{48}$/.test(config.token)
        ...
        || fs.realpathSync(config.directory) !== config.directory)
        throw new Error('安装控制请求不属于当前 DSH。');
    ...
    if (manifest.name !== '@deepseek-ai/dsh' || process.execArgv.length)
        throw new Error('此 DSH 启动方式尚不支持自动更新。');
    ...
    const read = async (method, payload = {}) => {
        if (!['session.list', 'session.history'].includes(method))
            throw new Error('不支持的安装检查');
        ...
        const response = gateway
            ...
            : await (await fetch(`http://127.0.0.1:${webPort}/api/${method}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(request), signal: AbortSignal.timeout(10000), redirect: 'error' })).json();
        if (!response.result?.ok)
            throw new Error('无法验证 DSH 会话状态，未停止节点。');
        return response.result.value;
    };
    const version = () => {
        try {
            return JSON.parse(fs.readFileSync(path.join(profile, 'node_modules/@harness-remote/dsh-wechat-remote/package.json'), 'utf8')).version;
        }
        catch {
            return '0.0.0';
        }
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (!['127.0.0.1', '::ffff:127.0.0.1'].includes(req.socket.remoteAddress || '')
            || req.headers.host !== `127.0.0.1:${server.address().port}` || req.headers['x-forwarded-for']
            || req.headers.authorization !== `Bearer ${config.token}` || req.method !== 'POST')
            return json(403, {});
        try {
            let body = '';
            ...
            if (req.url === '/describe')
                return json(200, { pid: process.pid, cli, executable: process.execPath,
            ...
            if (req.url === '/read' && !quiesced)
                return json(200, await read(input.method, input.payload));
            if (req.url === '/health' && !quiesced) {
                if (!gateway)
                    throw new Error('DSH 业务接口尚未就绪');
            ...
            if (req.url === '/launch' && !launched && !quiesced) {
                ...
                if (job.id !== id || job.controlOrigin !== origin || job.statusToken !== config.token || job.parentPid !== process.pid
                    || job.home !== home || job.profile !== profile || job.cli !== cli || job.pnpm !== config.pnpm)
                    throw new Error('安装目标发生变化');
            ...
            if (req.url === '/shutdown' && quiesced) {
                ...
                setTimeout(() => { close(); if (nativeExit)
                    nativeExit(0);
                else
                    process.exit(0); }, 50);
                return;
    ...
    const timer = setTimeout(() => { if (!launched)
        close(); }, 120000);
    timer.unref();
