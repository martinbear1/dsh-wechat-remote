...
export function runNativePlugin(cli, profile, home, toolPath, runtime, logFile, archiveName, timeoutMs = 600000, operation = 'add') {
...
export async function installProfile(job) {
    ...
    try {
        ...
        if (!(error instanceof NativeInstallError) || error.mayStillBeRunning || !layoutError.test(fs.readFileSync(logFile, 'utf8')))
            throw error;
