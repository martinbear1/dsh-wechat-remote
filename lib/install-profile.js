...
export function assertRelocatableProfile(root) {
...
export function runNativePlugin(cli, profile, home, toolPath, runtime, logFile) {
    if (!safeProfileName(profile))
        throw new Error('无效的 DSH profile 名称。');
...
export async function stageProfile(job) {
    const scope = path.basename(job.profile);
    if (!safeProfileName(scope) || !fs.statSync(job.directory).isDirectory())
    ...
    if (fs.existsSync(job.profile)) {
        if (fs.lstatSync(job.profile).isSymbolicLink() || fs.realpathSync(job.profile) !== path.resolve(job.profile))
            throw new Error('不支持自动替换链接形式的 profile。');
    ...
    if (fs.existsSync(filename)) {
        ...
        for (const [name, spec] of Object.entries(before.dependencies || {})) {
            if (name !== PLUGIN_PACKAGE && !/^[~^]?\d+\.\d+\.\d+(?:-[\w.-]+)?$/.test(String(spec)))
                throw new Error('其他插件使用了本地链接或特殊来源，未修改当前安装。');
        }
        for (const e of fs.readdirSync(staged, { withFileTypes: true }))
            if (e.isSymbolicLink())
                throw new Error('profile 配置包含链接，未修改当前安装。');
        if (before.packageManager && !/^pnpm@\d+\.\d+\.\d+(?:\+.*)?$/.test(before.packageManager))
            throw new Error('当前 profile 使用了其他包管理器，未修改安装。');
        writePrivateJsonAtomic(filename, { ...before, packageManager: `pnpm@${INSTALL_PNPM_VERSION}` });
    }
    ...
    for (const name of Object.keys(before?.dependencies || {}))
        if (name !== PLUGIN_PACKAGE) {
            if (hash(path.join(job.profile, 'node_modules', name, 'package.json')) !== hash(path.join(staged, 'node_modules', name, 'package.json')))
                throw new Error('安装试图改变其他插件，原安装保持不变。');
        }
