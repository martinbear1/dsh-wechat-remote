...
function base64url(value, bytes, name) {
...
export function validateRemoteAttachment(value) {
...
export function encryptCloudObject(plaintext, contentKind) {
    if (!plaintext.length || plaintext.length > MAX_CLOUD_OBJECT_BYTES)
        throw new Error('Cloud object size is invalid');
    ...
    for (let index = 0; index < chunks; index += 1) {
        ...
        const sealed = nacl.secretbox(plaintext.subarray(start, Math.min(start + ATTACHMENT_CHUNK_BYTES, plaintext.length)), counterNonce(noncePrefix, index), key);
...
export function decryptCloudObject(ciphertext, raw) {
    ...
    while (inputOffset < ciphertext.length) {
        if (inputOffset + 4 > ciphertext.length)
            throw new Error('Encrypted cloud object chunk header is truncated');
        ...
        if (length < nacl.secretbox.overheadLength || length > ATTACHMENT_CHUNK_BYTES + nacl.secretbox.overheadLength ||
            inputOffset + length > ciphertext.length)
            throw new Error('Encrypted cloud object chunk is invalid');
        const clear = nacl.secretbox.open(ciphertext.subarray(inputOffset, inputOffset + length), counterNonce(prefix, counter), key);
        if (!clear || outputOffset + clear.length > output.length)
            throw new Error('Encrypted cloud object authentication failed');
        output.set(clear, outputOffset);
        outputOffset += clear.length;
        inputOffset += length;
        counter += 1;
    }
    if (outputOffset !== output.length || counter === 0)
        throw new Error('Encrypted cloud object plaintext length mismatch');
    return output;
}
export function decryptRemoteAttachment(ciphertext, raw) {
