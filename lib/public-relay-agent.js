...
function transportDelay(value, fallback) {
...
function replaceAgentIdentity(identityPath) {
...
export class PublicRelayAgent {
     ...
     */
    async ensurePairingTicket(minValidityMs = 60_000, options = {}) {
    ...
    async enrollAndConnect() {
        ...
        try {
            ...
            const body = await this.enrollWithIdentityRecovery();
            if (this.stopped || revision !== this.lifecycleRevision)
                return;
            this.update({
                pairingTicket: body.ticket,
                pairingExpiresAt: body.expiresAt,
    ...
    async enrollWithIdentityRecovery() {
        const attemptedNodeId = this.identity.nodeId;
        try {
            return await this.enroll();
        }
        catch (error) {
            if (!(error instanceof RelayEnrollmentError) || error.code !== 'agent_revoked')
                throw error;
            ...
            if (this.identity.nodeId === attemptedNodeId)
                this.rotateRevokedIdentity();
            return this.enroll();
        }
    }
    rotateRevokedIdentity() {
        this.clearHeartbeat?.();
        this.clearHeartbeat = null;
    ...
    connect() {
        ...
        const clearHeartbeat = () => {
            ...
            pendingPing = null;
            if (this.clearHeartbeat === clearHeartbeat)
                this.clearHeartbeat = null;
        };
        ...
        const schedulePing = () => {
            ...
            pingTimer = setTimeout(() => {
                ...
                pongTimer = setTimeout(() => failTransport('Relay heartbeat timed out'), transportDelay(this.options.transportTiming?.pongTimeoutMs, RELAY_PONG_TIMEOUT_MS));
                pongTimer.unref?.();
                try {
                    ...
                    socket.ping(pendingPing, undefined, error => {
                        if (error)
                            failTransport('Relay heartbeat send failed');
                    });
                }
                catch {
                    failTransport('Relay heartbeat send failed');
        ...
        socket.on('pong', data => {
            if (this.socket !== socket || !pendingPing || !pendingPing.equals(data))
                return;
            if (pongTimer)
                clearTimeout(pongTimer);
            pongTimer = null;
 ...
 */
export function publicPairingPayload(status, lan) {
