...
export class NotificationRelayClient {
    ...
    async call(action, values) {
        ...
        if (!response.ok)
            throw Object.assign(new Error('提醒服务暂不可用，未改变任务状态'), { status: response.status });
...
export class TaskNotifications {
    ...
    start() {
        if (this.disposed || this.disposers.length)
            return;
    ...
    prepareWatch(watch) {
        ...
        watch.preparing = this.relay.call('prepare', { id: watch.id, epoch: this.clientEpoch,
            ...
            if ([400, 403, 404, 410].includes(error?.status))
                this.watches.delete(watch.id);
            throw error;
