...
const append = (e) => e.surfaceOp === undefined || e.surfaceOp === 'append';
...
function processEvidence(e) {
    const d = record(e.data);
    if (e.type === 'tool/call' || e.type === 'llm/retry' || e.type === 'tool/result' && append(e))
        return true;
    const visible = (b) => b.type !== 'tool-call' &&
        (b.type === 'text' || b.type === 'reasoning' ? typeof b.text === 'string' && b.text.trim() !== '' : true);
    if (e.type === 'assistant/message' && append(e))
        return (d.message?.content || []).some(visible);
    if (e.type !== 'assistant/live-chunk')
        return false;
    ...
    if (c.type === 'block-start')
        return !['text', 'reasoning', 'tool-call'].includes(c.blockType);
...
export class TurnActivityCompatibility {
    ...
    accept(event, facts) {
        ...
        if (event.type === 'turn/start')
            this.state = { turn: d.turn, start: seq, calls: new Map(), results: new Set(), changes: [], messages: [], visible: [], contexts: [], humans: [], tools: [], subagents: [], answer: null, reasoning: false, step: null, evidence: false };
        const s = this.state;
        if (!s || d.turn !== undefined && d.turn !== s.turn)
            return;
        ...
        if (event.type === 'assistant/message' && (event.surfaceOp === undefined || event.surfaceOp === 'append')) {
            ...
            if (d.step === s.step) {
                ...
                s.reasoning = content.some((b) => b.type === 'reasoning' && b.text?.trim());
            }
        }
        ...
        const hasProcess = tools.length || subagents.length || s.contexts.length || s.reasoning || s.visible.some((m) => m.step < s.step);
