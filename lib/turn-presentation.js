...
const fields = (v, names) => Object.fromEntries(names.flatMap(([from, to]) => number(v[from]) ? [[to, v[from]]] : []));
let nativeUsage;
...
function timing(events, start, end, firstTokens) {
    const result = {};
    if (number(start.time) && number(end.time) && end.time >= start.time)
        result.elapsedMs = end.time - start.time;
    let open, firstStep = Infinity, firstTokenMs;
    let decodeMs = 0, output = 0;
    for (const event of events) {
        const data = record(event.data);
        if (event.type === 'step/start')
            open = { step: data.step, time: event.time };
            ...
            if (token && open.first === undefined && number(event.time))
                open.first = event.time;
        }
            ...
            if (open && open.step === data.step && open.first === undefined && number(compactFirst) && compactFirst >= open.time)
                open.first = compactFirst;
    ...
    if (decodeMs > 0 && number(output / (decodeMs / 1000)))
        result.outputTokensPerSecond = output / (decodeMs / 1000);
    return result;
}
...
export function turnDetails(entries, usageFold, firstTokens) {
    const turns = new Map();
    for (const entry of entries) {
        ...
        group.push(event);
        turns.set(turn, group);
    }
    const details = [];
    for (const [turn, events] of turns) {
        const starts = events.filter(e => e.type === 'turn/start'), ends = events.filter(e => e.type === 'turn/end');
        if (starts.length !== 1 || ends.length !== 1 || events[0] !== starts[0] || events.at(-1) !== ends[0])
            continue;
        const closing = [...events].reverse().find(e => e.type === 'assistant/message' && typeof e.data?.message?.id === 'string'
            && Array.isArray(e.data.message.content) && e.data.message.content.some((b) => b.type === 'text' && typeof b.text === 'string' && b.text.trim()));
        if (!closing)
            continue;
        const value = { turnId: String(turn), messageId: closing.data.message.id };
        ...
        let usage;
        try {
            usage = usageFold?.(events);
        }
        catch { /* unavailable native reading */ }
        if (usage && number(usage.totalTokens)) {
            ...
            if (Array.isArray(usage.routes))
                value.usage.models = usage.routes.map(r => record(r)).filter(r => typeof r.provider === 'string' && typeof r.model === 'string').map(r => ({ provider: r.provider, model: r.model }));
        }
        if (value.usage || value.timing)
            details.push(value);
    }
    return details;
}
