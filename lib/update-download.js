...
import { trustedReleaseAsset, trustedNpmInstaller } from './update-policy.js';
    ...
    for (let i = 0; i < 5; i++) {
        ...
        try {
            ...
            try {
                ...
                options.signal?.throwIfAborted();
                throw new DownloadUnavailableError(undefined, { cause });
            }
            finally {
...
export async function boundedFetch(url, maxBytes, fetcher = fetch) {
    ...
    return fetchBytes(url, maxBytes, fetcher, [...githubHosts, 'relay.xyxfood.xyz'], Date.now() + (maxBytes <= 256 * 1024 ? 10000 : 60000), {});
}
function verifyBytes(archive, expected) {
    if (!expected || archive.length !== expected.bytes
        || createHash('sha256').update(archive).digest('hex') !== expected.sha256)
...
export function auditArchive(archive, release) {
    ...
    if (!manifest || manifest.name !== '@harness-remote/dsh-wechat-remote' || manifest.version !== release.version
        || !files.has('package/lib/index.js') || !files.has('package/lib/client.js'))
...
export function pluginFromInstaller(archive, release) {
    if (!trustedNpmInstaller(release.npmInstaller))
        throw new Error('备用安装包来源不受信任');
    ...
    visitArchive(archive, (name, data) => {
        ...
        if (name === 'package/assets/plugin.tgz') {
            if (data.length !== release.asset?.bytes)
                throw new Error('内置插件大小不匹配');
    ...
    if (!manifest || manifest.name !== 'dsh-wechat-remote' || manifest.version !== release.npmInstaller.version || !plugin)
        throw new Error('备用安装包名称、版本或内置插件不匹配');
    auditArchive(plugin, release);
    return plugin;
}
/** Also used by the publication gate to verify the alternate independently. */
export async function downloadNpmRelease(release, fetcher = fetch, options = {}) {
    if (!trustedReleaseAsset(release.asset, release.version) || !trustedNpmInstaller(release.npmInstaller))
        throw new Error('备用安装包来源不受信任');
...
export async function downloadRelease(release, fetcher = fetch, options = {}) {
    if (!trustedReleaseAsset(release.asset, release.version))
        throw new Error('暂无可验证的正式更新包');
    if (release.npmInstaller !== undefined && !trustedNpmInstaller(release.npmInstaller))
        throw new Error('备用安装包来源不受信任');
    const deadline = Date.now() + (options.timeoutMs ?? 120000);
    let body;
    try {
        body = await fetchBytes(release.asset.url, release.asset.bytes, fetcher, githubHosts, Math.min(deadline, Date.now() + 60000), options);
    }
    catch (error) {
        options.signal?.throwIfAborted();
        if (!(error instanceof DownloadUnavailableError) || !release.npmInstaller)
            throw error;
        const remaining = deadline - Date.now();
        if (remaining <= 0)
            throw error;
