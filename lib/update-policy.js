...
export function compareVersions(a, b) {
    ...
    if (!x[4] || !y[4])
        return x[4] === y[4] ? 0 : !x[4] ? 1 : -1;
    const xp = x[4].split('.'), yp = y[4].split('.');
    for (let i = 0; i < Math.max(xp.length, yp.length); i++) {
        if (xp[i] === yp[i])
            continue;
        if (xp[i] === undefined || yp[i] === undefined)
            return xp[i] === undefined ? -1 : 1;
        const xn = /^\d+$/.test(xp[i]), yn = /^\d+$/.test(yp[i]);
        if (xn && yn)
            return BigInt(xp[i]) > BigInt(yp[i]) ? 1 : -1;
        if (xn !== yn)
            return xn ? -1 : 1;
...
export function trustedReleaseAsset(asset, version) {
    if (!asset || !/^[a-f0-9]{64}$/.test(asset.sha256) || !Number.isSafeInteger(asset.bytes)
        || asset.bytes < 1 || asset.bytes > 32 * 1024 * 1024)
        return false;
    try {
...
export function trustedNpmInstaller(source) {
    if (!source || !validVersion(source.version) || !/^[a-f0-9]{64}$/.test(source.sha256)
        || !Number.isSafeInteger(source.bytes) || source.bytes < 1 || source.bytes > 32 * 1024 * 1024)
        return false;
...
export function validateCatalog(value) {
    ...
    if (!c || c.schemaVersion !== 1 || typeof c.revision !== 'string' || !/^[\w.-]{1,80}$/.test(c.revision)
        ...
        || !Array.isArray(c.blocked) || c.blocked.length > 100 || !strings(c.retiredDsh))
    ...
    for (const r of c.releases) {
        if (!r || !validVersion(r.version) || versions.has(r.version)
            ...
            || (r.npmInstaller !== undefined && (!r.asset || !trustedNpmInstaller(r.npmInstaller))))
        ...
        if (!hasEvidence && !pendingPreview)
            throw new Error('Invalid release evidence');
        versions.add(r.version);
        if (r.targets !== undefined) {
            if (!Array.isArray(r.targets) || !r.targets.length || r.targets.length > 12)
    ...
    for (const b of c.blocked)
        if (!b || !validVersion(b.pluginVersion)
            || typeof b.reason !== 'string' || !b.reason || b.reason.length > 240
            || (b.dsh && !strings(b.dsh)) || (b.platforms && !strings(b.platforms, 3)))
            throw new Error('Invalid blocked entry');
    if (!c.retiredDsh.every(validVersion))
        throw new Error('Invalid retired DSH');
    if (c.manualUpgradePlugins && (!strings(c.manualUpgradePlugins) || !c.manualUpgradePlugins.every(validVersion)))
        throw new Error('Invalid manual-upgrade plugins');
    return c;
}
export function releaseMatches(r, current) {
...
export function assessUpdate(raw, current, now = Date.now(), preview = false) {
    const base = { schemaVersion: 1, revision: '', checkedAt: now, expiresAt: now,
    ...
    if (c.expiresAt <= now || c.issuedAt > now + 300000)
        return { ...base, code: 'stale', label: '检查信息已过期' };
    if (!['dsh', 'deepseek-harness'].includes(current.agentKind))
        return { ...base, code: 'unknown-agent', label: '尚未提供兼容信息' };
    if (!validVersion(current.agentVersion) || !validVersion(current.pluginVersion)
        || !['windows', 'macos', 'linux'].includes(current.platform))
        return { ...base, code: 'missing-version', label: '版本信息不完整' };
    const blocked = (version) => c.blocked.find(b => b.pluginVersion === version
    ...
    if (c.retiredDsh.includes(current.agentVersion))
        return { ...base, severity: 'required', component: 'agent', code: 'agent-retired', label: '需要更新 DSH',
    ...
    if (issue)
        return { ...base, severity: 'required', component: 'plugin', code: 'blocked-no-target', label: '插件需处理',
            message: `${issue.reason}暂无可安全自动安装的匹配版本，请查看发布说明；不会自动升级或降级 DSH。`, releaseUrl: `${RELEASE_REPOSITORY}/releases` };
    if (compatible)
        return { ...base, severity: 'none', code: 'compatible', label: '暂无可用更新', message: '当前组合已验证，暂未发现适用于此 DSH 和主机的更新。' };
