...
export function validateJob(job) {
    if (!/^[a-f0-9]{32}$/.test(job.id) || path.basename(job.directory) !== job.id
        ...
        || job.argv[0] !== job.cli || !/^[\w.+-]{1,80}$/.test(job.targetVersion))
...
function durableSnapshot(job) {
    ...
    const walk = (dir) => {
        ...
        for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (e.isSymbolicLink())
                result[path.relative(job.home, file)] = createHash('sha256').update(fs.readlinkSync(file)).digest('hex');
 ...
 */
export function captureCandidateLock(home, child) {
    ...
    try {
        ...
        if (!owned && value === '' && process.platform === 'linux') {
            ...
            owned = fs.readdirSync(directory).some(fd => {
                try {
                    ...
                    if (fs.readlinkSync(file) !== filename)
                        return false;
        ...
        if (owned)
            return { filename, dev: info.dev, ino: info.ino, pid: child.pid };
    }
    catch { /* No verifiable owned lock: do not touch any lock. */ }
}
export function retireCandidateLock(lock, child, directory) {
    if (!lock || child.pid !== lock.pid || (child.exitCode === null && child.signalCode === null))
        return;
    try {
        const info = fs.lstatSync(lock.filename);
        if (!info.isFile() || info.isSymbolicLink() || info.nlink !== 1 || info.dev !== lock.dev || info.ino !== lock.ino || info.size > 32)
            return;
        ...
        if (!fs.existsSync(saved))
            fs.renameSync(lock.filename, saved);
    }
...
export async function executeUpdate(job, progress, quiesce) {
    ...
    try {
        ...
        if (error instanceof NativeInstallError && error.mayStillBeRunning)
            return {
        ...
        if (stopped || disposed) {
            ...
            try {
                ...
                else if (modified && job.manager && job.manager.kind !== 'process')
                    stopManagedHost(job.manager);
                if (modified) {
...
async function workerMain(filename) {
    ...
    // Attention may mean a package manager still owns the files. Leave the lock
    // and evidence in place instead of letting a second installer race with it.
