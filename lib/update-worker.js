...
export function validateJob(job) {
    ...
    for (const f of [job.executable, job.cli, job.pnpm, job.stateFile])
        if (!fs.statSync(f).isFile())
            throw new Error('安装运行时已变化');
}
function run(job, args, cwd) {
...
export async function healthy(job, version, timeoutMs = 60000) {
...
export async function executeUpdate(job, progress, quiesce) {
    ...
    let stopped = false, swapped = false, newChild;
    ...
    try {
        ...
        for (const [name, spec] of Object.entries(manifest.dependencies || {})) {
            if (name !== PLUGIN && !/^[~^]?\d+\.\d+\.\d+(?:-[\w.-]+)?$/.test(String(spec)))
                throw new Error('此 profile 含需手工处理的依赖来源');
        }
        for (const e of fs.readdirSync(staged, { withFileTypes: true }))
            if (e.isSymbolicLink())
                throw new Error('此 profile 配置包含链接，需手工更新');
        ...
        if (!within(staged, fs.realpathSync(installed)))
            throw new Error('暂存插件不在更新目录内');
        if (JSON.parse(fs.readFileSync(path.join(installed, 'package.json'), 'utf8')).version !== job.targetVersion)
            throw new Error('安装后插件版本校验失败');
        for (const name of Object.keys(manifest.dependencies))
            if (name !== PLUGIN) {
                ...
                if (hashFile(oldFile) !== hashFile(newFile))
                    throw new Error('暂存安装试图改变其他插件，需手工更新');
            }
        emit('checking', 50, '确认会话空闲并保存状态');
        if ((await describe(job)).pluginVersion !== job.previousVersion)
...
async function workerMain(filename) {
    ...
    validateJob(job);
    if (process.ppid !== job.parentPid || !process.connected)
    ...
    const quiesce = () => new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => { server.close(); if (process.connected)
        process.disconnect(); }, 120000);
    timer.unref();
}
