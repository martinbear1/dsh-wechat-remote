...
for (const version of ['0.1.2-rc.1', '0.1.5-rc.1', '0.1.6-rc.1', '0.1.7-rc.1']) {
  ...
  assert.equal(assessUpdate({ ...catalog, releases: [{ ...release, dsh: ['0.1.1-rc.2'] }] }, advice.current, now).targetVersion, undefined,
    'host capability alone must not invent target compatibility')
}
...
console.log('PASS version-independent native save/fence/dispose probe; future +1/+2 host catalog upgrades; missing capabilities and unverified targets fail closed')
