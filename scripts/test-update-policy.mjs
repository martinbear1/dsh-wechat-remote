...
import { assessUpdate, validateCatalog, compareVersions, trustedReleaseAsset, releaseMatches } from '../lib/update-policy.js'
...
const catalog = { schemaVersion: 1, revision: 'test-1', issuedAt: now - 1000, expiresAt: now + 86400000,
  ...
  blocked: [{ pluginVersion: '1.5.5', dsh: ['0.1.2-rc.1'], reason: 'Confirmed native protocol incompatibility' },
    { pluginVersion: '1.8.0', dsh: ['0.1.1-rc.2', '0.1.2-rc.1'], reason: 'Fixture: confirmed breaking change on these two hosts' }], retiredDsh: ['0.0.1'] }
...
test('known breaking target skipped in favor of safe newer plugin', () => {
...
test('explicitly broken 1.8 is not offered to an affected host', () => assert.equal(assessUpdate(catalog, { ...current, pluginVersion: '1.7.0' }, now).code, 'compatible'))
for (const agentVersion of ['99.0.0', '0.1.2-alpha.1', '0.1.5-rc.2']) test('untested host may update: ' + agentVersion, () => {
  const runtime = { ...current, agentVersion }
  const advice = assessUpdate(catalog, runtime, now)
  assert.equal(advice.targetVersion, '1.8.0')
  assert.equal(advice.component, 'plugin')
  assert.doesNotMatch(advice.message, /已支持|已验证|必须升级.*DSH/)
  assert.equal(releaseMatches(catalog.releases[2], runtime), false)
})
...
test('legacy 1.5.5 without updater or architecture still receives an actionable plugin badge', () => {
  ...
  for (const platform of ['windows', 'macos', 'linux']) {
    ...
    assert.equal(advice.label, '必须更新插件'); assert.equal(advice.component, 'plugin')
...
test('untested CPUs are offered the release without claiming tested evidence', () => {
  for (const platform of ['windows', 'macos', 'linux']) for (const arch of ['arm64', 'arm', 'riscv64', 's390x', 'futurecpu']) {
    const runtime = { ...current, platform, arch, agentVersion: '99.0.0' }
    const advice = assessUpdate(catalog, runtime, now)
    assert.equal(advice.targetVersion, '1.8.0')
    assert.doesNotMatch(advice.message, /已支持|已验证/)
  }
})
test('missing architecture does not suppress updates or invent evidence', () => {
  const advice = assessUpdate(catalog, { ...current, arch: '' }, now)
  assert.equal(advice.targetVersion, '1.7.0')
  assert.equal(assessUpdate(catalog, { ...current, arch: '', pluginVersion: '1.7.0' }, now).code, 'no-update')
  assert.equal(releaseMatches(catalog.releases[1], { ...current, arch: '' }), false)
})
...
test('withdrawn targets excluded', () => assert.equal(assessUpdate({ ...catalog, blocked: [{ pluginVersion: '1.7.0', reason: 'Withdrawn' }, { pluginVersion: '1.8.0', reason: 'Withdrawn' }] }, current, now).targetVersion, undefined))
