...
assert(installer && stable, 'Pass packed installer and exact release plugin tarball')
...
const plugin = JSON.parse(extract(stable, 'package/package.json'))
assert.equal(plugin.name, '@harness-remote/dsh-wechat-remote')
assert(payload.equals(stableBytes), 'Embedded payload must equal the release artifact byte for byte')
