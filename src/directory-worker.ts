...
function inputPath(): string {
  ...
  if (!path.isAbsolute(value)) throw new Error('absolute path required')
...
async function list(target: string): Promise<void> {
  ...
  const entries: Array<{ name: string; hidden: boolean }> = []
  let truncated = false
  ...
  try {
    for await (const dirent of dir) {
      ...
      if (!directory && dirent.isSymbolicLink()) {
        try { directory = (await stat(path.join(target, dirent.name))).isDirectory() } catch { directory = false }
      }
...
async function create(target: string): Promise<void> {
  try {
    await mkdir(target)
  } catch (error) {
    if (error && typeof error === 'object' && 'code' in error && error.code === 'EEXIST') process.exit(17)
    throw error
  }
}
...
async function main(): Promise<void> {
  ...
  if (action === 'list') return await list(target)
  if (action === 'create') return await create(target)
