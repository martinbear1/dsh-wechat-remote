...
export type LegacyRpcResult =
  ...
  | { readonly ok: false; readonly error: LegacyRpcError }
...
function present(value: JsonRecord, key: string): JsonRecord {
  return Object.hasOwn(value, key) ? { [key]: value[key] } : {}
}

function requestInvocation(method: string, payload: JsonRecord): InvocationPlan {
 ...
 */
export function planLegacyRpc(request: LegacyClientRequest): InvocationPlan {
  ...
  if (method.startsWith('goal.')) {
    ...
    const args: JsonRecord = { agentId: payload.sessionId }
    const mutation = {
...
function historyEvents(records: unknown): { readonly event: unknown }[] {
...
export function unpackChunkRow(event: JsonRecord): JsonRecord[] {
  ...
  if (!Array.isArray(members) || members.length === 0
      || members.some(item => typeof item !== 'string')) return []
  const gaps = Array.isArray(data.dt) ? data.dt : []
  if (gaps.length !== members.length - 1
      || gaps.some(gap => !Number.isSafeInteger(gap))) return []
...
async function permissionCommandValue(
  ...
  flushPermission?: (sessionId: string) => Promise<void>,
): Promise<unknown> {
...
export async function invokeLegacyPermissionRpc(
  gateway: Pick<TypertGatewayLike, 'invoke'>,
  request: LegacyClientRequest,
  signal: AbortSignal,
  readHistory: (sessionId: string, signal: AbortSignal) => Promise<unknown>,
  flushPermission?: (sessionId: string) => Promise<void>,
): Promise<LegacyServerResponse | null> {
  let result: LegacyRpcResult
  try {
    const plan = planLegacyRpc(request)
    if (plan.kind !== 'permission-command') return null
...
async function sessionModelsValue(
  gateway: TypertGatewayLike,
  request: JsonRecord,
  signal: AbortSignal,
): Promise<unknown> {
...
export async function invokeLegacyRpc(
  gateway: TypertGatewayLike,
  request: LegacyClientRequest,
  options: {
    readonly signal: AbortSignal
      ...
      for (let attempt = 0; ; attempt++) {
        try {
          ...
          if (request.method !== 'session.list' || attempt >= 2
              ...
              || !/ENOENT.*scandir.*[\\/]\.dsh-mkdir-[^\\/]+$/.test(failure.message)) throw error
...
export function parseLegacyClientRequest(
  pathMethod: string,
  value: unknown,
): LegacyClientRequest {
  ...
  return body as unknown as LegacyClientRequest
}
