...
export function adapterDshHome(
  environment: { readonly DSH_HOME?: string } = process.env,
  userHome = homedir(),
): string {
 ...
 */
export function resolveDshWebRuntime(
  ctx: Context,
  environment: DshRuntimeEnvironment = process.env,
): DshWebRuntime {
...
export function isAllowedDshWebOrigin(origin: string | undefined, port: number): boolean {
