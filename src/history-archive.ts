...
function crc32(data: Uint8Array): number {
  ...
  for (const byte of data) value = values[(value ^ byte) & 255] ^ (value >>> 8)
  return (value ^ 0xffffffff) >>> 0
}

function localHeader(crc: number, compressedBytes: number, clearBytes: number): Buffer {
...
function centralHeader(crc: number, compressedBytes: number, clearBytes: number): Buffer {
...
function endRecord(centralBytes: number, centralOffset: number): Buffer {
...
export function archiveHistoryJson(payloadJson: string): Uint8Array {
  const clear = Buffer.from(payloadJson, 'utf8')
  if (!clear.length) throw new Error('History snapshot is empty')
  const compressed = deflateRawSync(clear, { level: 6 })
  const crc = crc32(clear)
  return zip(clear, compressed, crc)
}

/** Runtime path: compression and checksum must not monopolize DSH's event loop. */
export async function archiveHistoryJsonAsync(payloadJson: string, signal: AbortSignal): Promise<Uint8Array> {
  signal.throwIfAborted()
  const clear = Buffer.from(payloadJson, 'utf8')
  if (!clear.length) throw new Error('History snapshot is empty')
  const compressed = await deflate(clear, { level: 6 })
  signal.throwIfAborted()
  const values = table()
  let value = 0xffffffff
  for (let offset = 0; offset < clear.length; offset += 256 * 1024) {
    const end = Math.min(clear.length, offset + 256 * 1024)
    for (let i = offset; i < end; i++) value = values[(value ^ clear[i]) & 255] ^ (value >>> 8)
    if (end < clear.length) await new Promise<void>(resolve => setImmediate(resolve))
    signal.throwIfAborted()
  }
  return zip(clear, compressed, (value ^ 0xffffffff) >>> 0)
}

function zip(clear: Buffer, compressed: Buffer, crc: number): Uint8Array {
