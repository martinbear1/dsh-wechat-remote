...
import { historyDetailDocument, historyRecordPreview, type DetailPart } from './history-record-presentation.js'
...
export interface HistoryDetailRequest {
  readonly sessionId: string; readonly reference: string; readonly part?: number; readonly offset?: number
}
...
type Snapshot = { parts: { metadata: Omit<DetailPart, 'text'>; text: Buffer }[]; bytes: number; digest: string }
const failure = (message: string) => Object.assign(new Error(message), { code: 'history-detail-unavailable' })
function snapshot(entry: Row, call?: Row): Snapshot {
...
export class HistoryRecords {
  ...
  private readonly cache = new Map<string, { value: Snapshot; expires: number }>()
  private cachedBytes = 0
  clear(): void { this.cache.clear(); this.cachedBytes = 0 }
  private remember(value: Snapshot): void {
    const existing = this.cache.get(value.digest)
    if (existing) { this.cachedBytes -= existing.value.bytes; this.cache.delete(value.digest) }
    const now = Date.now()
    for (const [key, entry] of this.cache) {
      if (entry.expires <= now || this.cachedBytes + value.bytes > CACHE_BYTES || this.cache.size >= 16) {
        this.cachedBytes -= entry.value.bytes; this.cache.delete(key)
      }
    }
    if (value.bytes <= CACHE_BYTES) {
      this.cache.set(value.digest, { value, expires: now + CACHE_TTL }); this.cachedBytes += value.bytes
    }
  }
  present(sessionId: string, entry: Row, displayed: Row = entry, call?: Row): Row {
    ...
    if (Buffer.byteLength(JSON.stringify(displayed)) <= HISTORY_RECORD_BYTES) return displayed
    const seq = entry.event?.seq
    if (!Number.isSafeInteger(seq) || seq < 0) throw failure('历史记录缺少原生序号')
    ...
    } satisfies HistoryDetailReference }
    if (Buffer.byteLength(JSON.stringify(partial)) > HISTORY_RECORD_BYTES) throw failure('记录的标识或关联信息超过手机单条展示上限，请在电脑端查看')
    return partial
  }
  async read(request: HistoryDetailRequest, load: (seq: number) => Promise<Row | undefined>, signal: AbortSignal): Promise<HistoryDetailPage> {
    ...
    if (typeof reference !== 'string' || reference.length > 4096 || !Number.isSafeInteger(offset) || offset < 0
      || !Number.isSafeInteger(part) || part < 0) throw failure('详情请求无效')
    const [payload, signature, extra] = reference.split('.')
    const actual = Buffer.from(signature || '', 'base64url')
    const expected = createHmac('sha256', this.secret).update(payload || '').digest()
    if (extra || actual.length !== expected.length || !timingSafeEqual(actual, expected)) throw failure('详情引用无效，请刷新会话')
    const identity = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'))
    if (identity.sessionId !== sessionId || identity.expires <= Date.now()) throw failure('详情引用已失效，请刷新会话')
    let cached = this.cache.get(identity.digest)
    if (!cached || cached.expires <= Date.now()) {
      const original = await load(identity.seq)
      signal.throwIfAborted()
      if (!original || original.event?.seq !== identity.seq) throw failure('原始记录已不可读取，请刷新会话')
      const call = identity.callSeq === undefined ? undefined : await load(identity.callSeq)
      signal.throwIfAborted()
      if (identity.callSeq !== undefined && call?.event?.seq !== identity.callSeq) throw failure('工具调用已不可读取，请刷新会话')
      const value = snapshot(original, call)
      if (value.bytes !== identity.bytes || value.digest !== identity.digest) throw failure('原始记录已变化，请刷新会话')
      this.remember(value); cached = { value, expires: Date.now() + CACHE_TTL }
    }
    const source = cached.value.parts[part]
    if (!source || offset > source.text.length) throw failure('详情位置无效')
    const bytes = source.text
    if (offset < bytes.length && (bytes[offset] & 0xc0) === 0x80) throw failure('详情位置不是有效文字边界')
