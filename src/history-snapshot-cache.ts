...
interface StoredEntry {
  ...
  readonly descriptor: HistorySnapshotDescriptor
  readonly expiresAt: number
}
...
export class HistorySnapshotCache {
  ...
  private readonly onDiagnostic?: HistorySnapshotCacheOptions['onDiagnostic']
  ...
  set(digest: string, rawDescriptor: unknown): HistorySnapshotDescriptor {
    if (!isDigest(digest)) throw new Error('History snapshot digest is invalid')
  ...
  private restore(): void {
    ...
    try {
      const stored = readPrivateJson<StoredCache>(this.file)
      ...
      for (const raw of stored.entries.slice(-MAX_CACHE_ENTRIES * 2)) {
        try {
          if (!raw || typeof raw !== 'object' || !isDigest(raw.digest)) throw new Error('invalid digest')
          const descriptor = validateHistorySnapshotDescriptor(raw.descriptor, now)
          if (raw.expiresAt !== descriptor.expiresAt) throw new Error('expiry mismatch')
  ...
  private prune(): void {
    ...
    for (const [digest, entry] of this.entries) {
      if (entry.expiresAt <= threshold) this.entries.delete(digest)
    }
  ...
  private persist(): void {
    if (!this.file || !this.persistenceEnabled) return
    try {
...
export function validateHistorySnapshotDescriptor(raw: unknown, now = Date.now()): HistorySnapshotDescriptor {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('History snapshot descriptor is invalid')
...
function validateBase64Url(value: unknown, bytes: number, name: string): void {
