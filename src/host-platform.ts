...
abstract class BasePlatformAdapter implements HostPlatformAdapter {
  abstract readonly descriptor: HostPlatformDescriptor

  abstract filesystemRoots(signal: AbortSignal): Promise<readonly PlatformFilesystemRoot[]>

  async isPotentiallyBlockingPath(_target: string, signal: AbortSignal): Promise<boolean> {
...
class WindowsPlatformAdapter extends BasePlatformAdapter {
  readonly descriptor: HostPlatformDescriptor = Object.freeze({
    kind: 'windows',
    name: 'Windows',
    pathStyle: 'windows',
  ...
  private rootsPromise?: Promise<readonly PlatformFilesystemRoot[]>
  private readonly rootsByPath = new Map<string, PlatformFilesystemRoot>()

  async filesystemRoots(signal: AbortSignal): Promise<readonly PlatformFilesystemRoot[]> {
  ...
  async isPotentiallyBlockingPath(target: string, signal: AbortSignal): Promise<boolean> {
    signal.throwIfAborted()
...
class MacPlatformAdapter extends BasePlatformAdapter {
  ...
  async filesystemRoots(signal: AbortSignal): Promise<readonly PlatformFilesystemRoot[]> {
    signal.throwIfAborted()
    const roots: PlatformFilesystemRoot[] = [
      { name: '主目录', path: homedir(), kind: 'home' },
      { name: '根目录', path: '/', kind: 'filesystem' },
    ]
    if (existsSync('/Volumes')) roots.push({ name: '卷', path: '/Volumes', kind: 'volume' })
    return roots
  }
...
class LinuxPlatformAdapter extends BasePlatformAdapter {
  ...
  async filesystemRoots(signal: AbortSignal): Promise<readonly PlatformFilesystemRoot[]> {
    ...
    for (const mountRoot of ['/mnt', '/media']) {
      if (existsSync(mountRoot)) roots.push({ name: path.basename(mountRoot), path: mountRoot, kind: 'volume' })
    }
    return roots
  }
...
function buildAdapter(): HostPlatformAdapter {
  switch (platform()) {
...
export const hostPlatform: HostPlatformAdapter = buildAdapter()

export function hostPlatformDescriptor(): HostPlatformDescriptor {
...
export function selectLanIPv4(
  source: Readonly<Record<string, readonly NetworkInterfaceInfo[] | undefined>> = networkInterfaces(),
): string {
...
function scoreAddress(candidate: NetworkCandidate): number {
  ...
  if (/(tailscale|docker|veth|vmnet|virtualbox|hyper-v|wsl|loopback|utun|bridge)/i.test(name)) score -= 80
  if (address.startsWith('169.254.') || isBenchmarkAddress(address)) score -= 200
  return score
}
...
async function enumerateWindowsRoots(signal: AbortSignal): Promise<readonly PlatformFilesystemRoot[]> {
  const script = [
  ...
  const unique = new Map<string, PlatformFilesystemRoot>()
