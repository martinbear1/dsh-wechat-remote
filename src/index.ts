...
import HistorySnapshotPrewarmer from './history-prewarmer.js'
...
let historySnapshotPrewarmer = null
...
function loadState() {
  try {
    ...
    if (raw && typeof raw.token === 'string' && raw.token.length >= 32) {
      ...
      if (!raw.wechatBindings) raw.wechatBindings = {}
      return raw
    }
  } catch (e) { /* first run */ }
...
proxy.on('proxyRes', (proxyRes, req, res) => {
  ...
  if (!alreadyEncoded && compressible && !noBody && /\bgzip\b/.test(accept)) {
    ...
    gzip.on('error', () => { if (!res.destroyed) res.destroy() })
...
function loadWechatConfig() {
  try {
    ...
    if (raw && typeof raw.appid === 'string' && typeof raw.secret === 'string') return raw
    ...
    const req = https.get('https://api.weixin.qq.com/sns/jscode2session?' + qs.toString(), (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (parsed.openid) return resolve(parsed.openid)
...
const publicServer = http.createServer((req, res) => {
  ...
  if (url.pathname === '/pair/claim-wechat') {
    if (!allowRequest(req, true)) { accessLog(req, 'DENIED-429'); return reject429(res) }
    accessLog(req, 'claim-wechat')
    return serveClaimWechat(req, res)
  }
  if (!allowRequest(req, false)) { accessLog(req, 'DENIED-429'); return reject429(res) }
...
export function apply(ctx) {
  ...
  ctx.plugin(WechatHistoryService, {
    ...
    prepareSnapshot: async payloadJson => {
      ...
      return publicRelayGateway.prepareHistorySnapshot(payloadJson)
    },
  })
  ...
  try {
    ...
    if (relayConfig) {
        ...
        issueLanCredential: (rotate = false) => {
          ...
          if (rotate) {
            ...
            for (const openId of Object.keys(state.wechatBindings)) {
              if (state.wechatBindings[openId] === previous) state.wechatBindings[openId] = rotated
            }
            saveState(state)
      ...
      historySnapshotPrewarmer = new HistorySnapshotPrewarmer({
        dshPort: UPSTREAM_PORT,
        warm: async (sessionId, signal) => {
          if (!ctx.wechatHistory) throw new Error('Wechat history service is unavailable')
          return prewarmLatestHistory(ctx.wechatHistory, sessionId, signal)
        },
        onDiagnostic: (level, message) => {
          if (level === 'warn') console.warn(`[wechat-gate] ${message}`)
          else console.log(`[wechat-gate] ${message}`)
  ...
  ctx.on('dispose', () => {
    ...
    try { historySnapshotPrewarmer?.stop() } catch (e) { /* best-effort */ }
    historySnapshotPrewarmer = null
