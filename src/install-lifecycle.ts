...
export type HostManager = { kind: 'process' } | { kind: 'systemd'; unit: string }
  | { kind: 'launchd'; label: string; domain: string; plist: string }
const serviceName = (s: string) => /^[A-Za-z0-9_.@-]{1,180}$/.test(s)
...
export function validateManager(value: HostManager): void {
  ...
  if (value.kind === 'launchd' && process.platform === 'darwin' && serviceName(value.label)
    ...
    && path.isAbsolute(value.plist) && fs.statSync(value.plist).isFile()) return
  throw new Error('无法确认原后台服务，未停止 DSH。')
}

/** Called INSIDE DSH, so pid and service ownership cannot be guessed by a client. */
export function currentHostManager(): HostManager {
  ...
  if (process.platform === 'darwin') {
    ...
    if (row) {
      ...
      const result: HostManager = { kind: 'launchd', label, domain, plist }
      validateManager(result); return result
    }
...
export function stopManagedHost(manager: HostManager): void {
...
export function startManagedHost(manager: HostManager): void {
  validateManager(manager)
...
export function startUpdateWorker(manager: HostManager, directory: string, executable: string): void {
  validateManager(manager)
