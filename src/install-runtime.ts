...
export interface InstallRuntime { executable: string; cli: string; version: string }
...
export async function pinInstallRuntime(runtime: InstallRuntime, directory: string): Promise<InstallRuntime> {
...
export function resolveInstallRuntime(owner: string, executable = process.execPath,
  nodeVersion = process.versions.node): InstallRuntime {
  ...
  try { filename = require.resolve('pnpm') }
  catch { throw new Error('安装工具不完整，请使用官方安装命令自动修复。') }
...
export function verifyInstallRuntime(runtime: InstallRuntime): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', b => { size += b.length; if (size > 16384) { child.kill(); finish(new Error('安装工具响应异常。')) } else stdout += b.toString() })
