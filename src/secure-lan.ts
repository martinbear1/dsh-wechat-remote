...
type AdmissionState = 'pending' | 'active'
...
export class SecureLanAdmissionPool {
  private readonly entries = new Map<symbol, { address: string; state: AdmissionState }>()
  private readonly pendingByAddress = new Map<string, number>()
  private pending = 0
  private active = 0
  ...
  authenticate(admission: SecureLanAdmission): boolean {
    ...
    if (!entry || entry.state !== 'pending' || entry.address !== admission.address) return false
    if (this.active >= (this.limits.maxActive ?? 8)) return false
  ...
  release(admission: SecureLanAdmission): void {
    const entry = this.entries.get(admission.id)
    if (!entry || entry.address !== admission.address) return
    this.entries.delete(admission.id)
    if (entry.state === 'pending') this.releasePending(entry.address)
    else this.active = Math.max(0, this.active - 1)
  }

  snapshot(): { readonly pending: number; readonly active: number; readonly addresses: number } {
    return { pending: this.pending, active: this.active, addresses: this.pendingByAddress.size }
  }

  private releasePending(address: string): void {
...
export class SecureLanServer {
  ...
  attach(ws: WebSocket, remoteAddress?: string): void {
    ...
    if (!identity) { ws.close(1013, 'LAN unavailable'); return }
    const admission = this.admissions.begin(remoteAddress)
    if (!admission) { ws.close(1013, 'LAN busy'); return }
    ...
    ws.on('message', (raw, binary) => {
      try {
        ...
        if (!tunnel) {
          ...
          if (!this.admissions.authenticate(admission)) throw new Error('LAN capacity reached')
