...
type Session = { id: string; events?: readonly Event[]; snapshotEvents?: (fromSeq?: number) => readonly Event[]; firstLiveSeq: number }
type Watch = { id: string; session: Session; turn: number; baseline: number; expiresAt: number; preparing?: Promise<any> }
type Context = { get(name: string): any; on(name: any, callback: any): () => void }
...
function events(session: Session, fromSeq = 0): readonly Event[] {
  const value = typeof session.snapshotEvents === 'function' ? session.snapshotEvents(fromSeq) : session.events
  if (!Array.isArray(value)) throw new Error('当前节点尚未提供可验证的任务通知事件')
...
export function currentTurn(session: Session): number | null {
  ...
  for (let i = snapshot.length - 1; i >= 0; i--) {
    ...
    if (event.type === 'turn/start') return Number.isSafeInteger(event.data?.turn) ? event.data.turn : null
  }
  return null
}

/** Body-bound proof, distinct domain from the legacy object API. */
export function notificationProof(path: string, node: string, time: number, nonce: string, body: string): Buffer {
  return Buffer.from(`task-notification-v1\nPOST\n${path}\n${node}\n${time}\n${nonce}\n${createHash('sha256').update(body).digest('hex')}`)
}

export class NotificationRelayClient {
  constructor(readonly origin: string, private identity: () => AgentIdentity, private fetchImpl: typeof fetch = fetch) {}
  async call(action: 'prepare' | 'observe', values: unknown): Promise<any> {
...
export class TaskNotifications {
  ...
  private questions = new Map<string, { session: Session; turn: number; signal?: AbortSignal }>()
  private presence = new Map<string, number>()
  private disposers: (() => void)[] = []
  private timer?: ReturnType<typeof setInterval>
  private pumping = false
  private disposed = false
  private clientEpoch = randomBytes(12).toString('base64url')
  private cursor = 0

  constructor(private ctx: Context, private relay: Pick<NotificationRelayClient, 'call' | 'origin'>,
  ...
  start(): void {
    ...
    this.disposers.push(this.ctx.on('tools/execute', (exec: any, next: () => Promise<any>) => {
      ...
      let key = ''
      try {
        const session = this.ctx.get('sessions')?.get?.(exec.agent?.id) as Session | undefined
      ...
      try { result = next() } catch (error) { if (key) this.questions.delete(key); throw error }
  ...
  private session(id: unknown): Session {
    if (typeof id !== 'string' || id.length > 128) throw new Error('请选择正在运行的会话')
  ...
  async request(args: any): Promise<any> {
    ...
    if (args.action === 'presence') {
      ...
      if (!this.presence.has(session.id) && this.presence.size >= 128) this.presence.delete(this.presence.keys().next().value!)
      if (args.visible === true) this.presence.set(session.id, this.now() + 25000)
    ...
    if (existing) return this.prepareWatch(existing)
    if (this.pending(session, turn).length) throw new Error('请先处理当前问答或授权，任务继续后可再次订阅')
    if (this.watches.size >= 16) throw new Error('待处理提醒过多，请稍后再试')
    const watch: Watch = { id: randomBytes(18).toString('base64url'), session, turn,
  ...
  private prepareWatch(watch: Watch): Promise<any> {
    if (watch.preparing) return watch.preparing
  ...
  private pending(session: Session, turn: number): { id: string; type: string }[] {
    const approvals = new Map<string, { id: string; type: string }>()
    let active: number | null = null
    for (const event of events(session)) {
      if (event.type === 'turn/start') active = event.data.turn
  ...
  snapshot(watch: Watch): any {
    ...
    if (live !== watch.session) return { state: 'cancelled' } // Dispose/reload is NOT completion.
    const end = events(watch.session, watch.baseline + 1).find(e => e.type === 'turn/end' && e.data.turn === watch.turn)
    if (end) return end.data.reason?.kind === 'completed'
      ? { state: 'complete', eventKey: `end:${end.seq}` } : { state: 'cancelled' }
    if (currentTurn(watch.session) !== watch.turn) return { state: 'cancelled' }
    {
      const items = this.pending(watch.session, watch.turn)
      if (items.length) return { state: 'pending', eventKey: createHash('sha256').update(items.map(i => i.id).sort().join('\n')).digest('hex'),
  ...
  async tick(): Promise<void> {
    ...
    try {
      for (const [id, expires] of this.presence) if (expires <= this.now()) this.presence.delete(id)
      for (const watch of this.watches.values()) if (watch.expiresAt <= this.now()) this.watches.delete(watch.id)
