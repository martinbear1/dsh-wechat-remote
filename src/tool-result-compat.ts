...
export function toolResultFailed(message: Row | undefined): boolean {
  return message?.isError === true || Array.isArray(message?.content)
    && message.content.some((block: Row) => block?.type === 'tool-result' && block.isError === true)
}
export function presentToolResult<T extends Row>(entry: T): T {
  const event = entry.event, message = event?.data?.message
  if (event?.type !== 'tool/result' || message?.role !== 'tool' || !Array.isArray(message.content)) return entry
  ...
  if (message.content.some((block: Row) => block?.type === 'tool-result')) return entry
  if (typeof message.toolCallId !== 'string' || message.source?.kind !== 'tool'
    || message.source.callId !== message.toolCallId) throw new Error('DSH 工具结果缺少匹配的调用标识')
