...
import { trustedReleaseAsset, type Release } from './update-policy.js'
...
export async function boundedFetch(url: string, maxBytes: number, fetcher = fetch): Promise<Buffer> {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (u.protocol !== 'https:' || u.username || u.password || u.port
        || !['github.com', 'release-assets.githubusercontent.com', 'objects.githubusercontent.com', 'relay.xyxfood.xyz'].includes(u.hostname)) throw new Error('更新下载来源不受信任')
    ...
    if (!res.ok || !res.body) { await res.body?.cancel(); throw new Error('更新服务暂不可用') }
...
export function auditArchive(archive: Buffer, release: Release): void {
  if (!release.asset || archive.length !== release.asset.bytes
      || createHash('sha256').update(archive).digest('hex') !== release.asset.sha256) throw new Error('更新包校验失败，未修改当前插件')
  const tar = gunzipSync(archive, { maxOutputLength: 64 * 1024 * 1024 })
  const seen = new Set<string>()
  let manifest: Record<string, unknown> | undefined, entries = 0
  for (let offset = 0; offset + 512 <= tar.length;) {
    ...
    if (!name.startsWith('package/') || /[\\:\x00-\x1f]/.test(name)
        ...
        || !Number.isSafeInteger(size) || size < 0 || offset + 512 + size > tar.length) throw new Error('更新包包含不安全的路径或文件类型')
  ...
  if (!manifest || manifest.name !== '@harness-remote/dsh-wechat-remote' || manifest.version !== release.version
      || !seen.has('package/lib/index.js') || !seen.has('package/lib/client.js')) throw new Error('更新包名称、版本或入口不匹配')
...
export async function downloadRelease(release: Release, fetcher = fetch): Promise<Buffer> {
