...
export function resumedWebUrl(connection: any, origin: string, webPort: number): string {
  if (!isAllowedDshWebOrigin(origin, webPort)) throw new Error('WebUI 地址不匹配')
  const value = typeof connection?.authenticatedUrl === 'function' ? connection.authenticatedUrl(origin) : origin + '/'
  const url = new URL(value)
  if (url.origin !== new URL(origin).origin || url.pathname !== '/' || url.username || url.password || url.hash) throw new Error('WebUI 恢复地址无效')
  return url.href
}
